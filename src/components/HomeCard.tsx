...
export type HomeCardType = '위로' | '공감' | '명언' | '현실조언' | 'Back';
...
export default forwardRef<View, HomeCardProps>(function HomeCard({
  ...
  return (
    <View ref={ref} style={[styles.card, { backgroundColor: CARD_BG[type] }]}>
      ...
      {/* 상단 행: Back이면 카테고리 배지, 아니면 공유 버튼 */}
      <View style={[styles.topRow, isBack && styles.topRowCenter]}>
        {isBack ? (
      ...
      {/* 중앙 텍스트 */}
      <View style={[styles.textArea, { paddingVertical: isBack ? 20 : 40 }]}>
        <AppText
          variant="sectionTitle"
          style={[
            styles.cardText,
            { color: textColor, maxHeight: isBack ? 189 : 144 },
          ]}
          numberOfLines={isBack ? 7 : 5}
        >
          {text}
        </AppText>
      </View>
